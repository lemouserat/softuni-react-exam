...
export const BlogPostDetails = () => {
  ...
  const onRecommendationDelete = async () => {
    const recommendation = blog.recommendations?.find(
      (x) => x._ownerId === userId
    );
  ...
  return (
    <>
      <Modal animation={false} show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          Think about everyone who would want to see your blog first!
        </Modal.Body>
        <Modal.Footer>
      ...
      <section id="video" className="content-section">
        <div className="row">
          <div className="col-md-12">
            ...
            <div className="col-md-12">
              <img
                className={styles.imageBlogDetails}
            ...
            <div className="text-content">
              <p>{blog.story}</p>
              <p className={styles.author}>Author: {blog.userEmail} </p>
            </div>
            {isAuthenticated && (
              <div>
                {!isOwner && (
            ...
            {isOwner && (
              <div className={styles.editDeleteButtons}>
                ...
                <div className={styles.deleteButton}>
                  <a href="#blog" onClick={() => handleClickDelete(blog._id)}>
                    Delete
                  </a>
                </div>
              </div>
                ...
                {blog.comments &&
                  blog.comments.map((x) => (
                    <li key={x._id} className="comment">
                      <p>
                        {" "}
                        <b>{x.comment}</b> <br /> by: {x.author.email}{" "}
                      </p>
                      {userEmail === x.author.email && (
                        <button
                          onClick={() => onCommentDelete(x._id)}
